...
def main():
    ...
    symbol_quantity = parse_formula(formula, periodic)
    print(symbol_quantity)

    #parse_formula(formula, periodic_table_dict)
    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    #compute_molar_mass(symbol_quantity_list, periodic_table_dict)
    molar_mass = compute_molar_mass(symbol_quantity, periodic)
    # Compute the number of moles in the sample.
    number_of_moles = float(sample_mass) / molar_mass
    #number_total= sample_mass / molar_mass
    # Print the molar mass.
    print(molar_mass)
    print(f" {molar_mass} grams/mole")
    # Print the number of moles.
    print(f" {number_of_moles:.5f} number of moles")
    dict= molecules_dict()

    get_formula_name(formula,dict)

    # sum = sum_protons(symbol_quantity_list,periodic_table_dict)
    # print(sum)
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    total_molar_mass = 0 #total_mass = 0
...
def get_formula_name(formula,known_molecules_dict):
    ...
    if formula in known_molecules_dict:
        formu = known_molecules_dict[formula]  
        print(f'The name is: {formu}')
    else:
        print('Unknow compound')
...
#     """
#     symbol_quantity_list.insert(1,'89')#serch a quickly way add items to list.
#     total_protobs = 0
#     for inner_list in symbol_quantity_list:
#         symbol = inner_list[SYMBOL_INDEX]
#         print(inner_list[1])
