...
def print_death_age(people_dict):
    ...
    for name, age in people_dict.items():
        shows_name = age[NAME_INDEX]

        shows_age = age[DEATH_YEAR_INDEX] - age[BIRTH_YEAR_INDEX] 
        print(f'Name: {shows_name}, Age at death: {shows_age} ')
...
def count_genders(people_dict):
    ...
    for key,value in people_dict.items():
        number = value[GENDER_INDEX]
        if number == "F":
            M_count += 1
    num_people = len(people_dict)
    F_count = num_people - M_count
            #M_count = M_count + 1

    print(f'the female number: {F_count}')
    print(f'the male number: {M_count}')

        
        
        #print(f'The number of males and females are: {number} ')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage_list in marriages_dict.items():
        # husband = age[HUSBAND_KEY_INDEX]
        # for name,year in people_dict.items():
        #     husband_name = name[NAME_INDEX]
        #     year = year[WEDDING_YEAR_INDEX]
        #     print(f'the name and age in the weedding year of the husband: {husband} {husband_name} {year}')
